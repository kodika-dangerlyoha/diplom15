...
const GameCardH = ({ game }) => {
	...
	return (
		<article className={`gameH game_${game.id}`} data-game_id={game.id}>
			<div className="gameH__banner">
				<div className="gameH__banner__imgBlock">
					{imageSrc ? <img src={imageSrc} data-lazy_src={imageSrc} alt={game.name} /> : null}
				</div>
				<div className="gameH__banner__videoBlock">
					<video autoPlay loop muted playsInline>
						{game.treiler_micro_webm ? <source src={game.treiler_micro_webm} type="video/webm" /> : null}
						{game.treiler_micro_mp4 ? <source src={game.treiler_micro_mp4} type="video/mp4" /> : null}
					</video>
				</div>
				<div className="gameH__banner__info txt">
					<Link href={href} className="gameH__banner__info__link" />
					<div className="gameH__banner__info__top">
						...
						<button
							...
							className={`gameH__info__buttons__button_mobile _buttonBasket ${
								isInBasket(game.id) ? 'gameH__info__buttons__button_mobile_active' : ''
							}`}
						...
						<button
							...
							data-game_id={game.id}
							className={`gameH__info__buttons__button_mobile _buttonFavorite ${
								isInFavorite(game.id) ? 'gameH__info__buttons__button_mobile_active' : ''
							}`}
